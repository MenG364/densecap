...
from .caption_head.caption_head import ROICaptionHead
...
class AttrRCNN(GeneralizedRCNN):
    ...
    def __init__(self, cfg):
        ...
        self.caption = ROICaptionHead(cfg,2048)
    ...
    def forward(self, images, targets=None):
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON:
            ...
            _, predictions, loss_attribute = self.attribute(
                attribute_features, predictions, targets
            )
            detector_losses.update(loss_attribute)
        CAPTION_ON = True
        if CAPTION_ON:
            predictions, loss_caption = self.caption(
                x, predictions, targets
            )
            detector_losses.update(loss_caption)
