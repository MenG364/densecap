...
def encode_boxes(data, image_data, all_image_ids):
    ...
    for i, img in enumerate(data):
        ...
        for region in img['regions']:
            ...
            # sanity check
            try:
                assert x1 <= img_info['width'], 'invalid x1 coordinate {} > {} in image_id:{} box_id:{}'.format(x1,
                                                                                                                img_info[
                                                                                                                    'width'],
                                                                                                                img[
                                                                                                                    'id'],
                                                                                                                region[
                                                                                                                    'region_id'])
                assert y1 <= img_info['height'], 'invalid y1 coordinate {} > {} in image_id:{} box_id:{}'.format(y1,
                                                                                                                 img_info[
                                                                                                                     'height'],
                                                                                                                 img[
                                                                                                                     'id'],
                                                                                                                 region[
                                                                                                                     'region_id'])
                assert x2 <= img_info['width'], 'invalid x2 coordinate {} > {} in image_id:{} box_id:{}'.format(x2,
                                                                                                                img_info[
                                                                                                                    'width'],
                                                                                                                img[
                                                                                                                    'id'],
                                                                                                                region[
                                                                                                                    'region_id'])
                assert y2 <= img_info['height'], 'invalid y2 coordinate {} > {} in image_id:{} box_id:{}'.format(y2,
                                                                                                                 img_info[
                                                                                                                     'height'],
                                                                                                                 img[
                                                                                                                     'id'],
...
def filter_images(data,regionid2cls, split_data):
    ...
    for img in data:
        ...
        if keep:
            new_data.append(img)
            new_regionid2cls[img['id']]=regionid2cls[img['id']]
    for d, c in zip(new_data, new_regionid2cls.keys()):
        if len(d['regions']) != len(new_regionid2cls[c]):
            regions=[]
            for region in d['regions']:
                if region['region_id'] in new_regionid2cls[c].keys():
    ...
    return new_data,new_regionid2cls
...
def main(args):
    ...
    with open('data/rig2cls.pkl','rb') as f:
        regionid2cls=pickle.load(f)
    ...
    data,regionid2cls = filter_images(data, regionid2cls, split_data)  # data为vg数据集定义的region description的格式的列表
...
if __name__ == '__main__':
    ...
    parser.add_argument('--region_data',
                        default='F:/datasets/VQA/VQA2.0old/data/visual-genome/region_descriptions.json',
                        help='Input JSON file with regions and captions')
    parser.add_argument('--image_data',
                        default='F:/datasets/VQA/VQA2.0old/data/visual-genome/image_data.json',
