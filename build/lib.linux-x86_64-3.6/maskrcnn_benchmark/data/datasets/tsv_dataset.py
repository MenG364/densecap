...
class TSVDataset(object):
    ...
    def __getitem__(self, idx):
        features, img_size = self.get_image(idx)
    ...
    def get_image(self, idx):
        ...
        hw=TSVFile(self.hw_file).seek(line_no)
        labels = TSVFile(self.label_file).seek(line_no)
        # use -1 to support old format with multiple columns.
        # img = img_from_base64(row[-1])
        row[1]=json.loads(row[1])
        img=row
        features = np.frombuffer(base64.b64decode(img[1]['features']), np.float32
                                 ).reshape((img[1]['num_boxes'], -1))
        features = torch.tensor(features)
        img_size=json.loads(hw[1])
        img_size=[img_size[0]['height'],img_size[0]['width']]
        labels=json.loads(labels[1])

        bbox=[]
        for box in labels:
            bbox.append(box['rect'])


        return features,img_size
...
class TSVYamlDataset(TSVDataset):
    ...
    def __init__(self, yaml_file):
        ...
        linelist_file = find_file_path_in_yaml(self.cfg.get('linelist', None),
                                               self.root)
        feature_file = find_file_path_in_yaml(self.cfg.get('feature', None),
                                              self.root)
        super(TSVYamlDataset, self).__init__(feature_file, label_file, hw_file, linelist_file)
