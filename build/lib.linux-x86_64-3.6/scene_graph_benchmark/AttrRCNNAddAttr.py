...
class AttrRCNNAddAttr(GeneralizedRCNN):
    """
    ...
    def __init__(self, cfg):
        ...
        super(AttrRCNNAddAttr, self).__init__(cfg)
        self.cfg = cfg
    ...
    def to(self, device, **kwargs):
        super(AttrRCNNAddAttr, self).to(device, **kwargs)
        if self.cfg.MODEL.ATTRIBUTE_ON:
            self.attribute.to(device, **kwargs)

    def forward(self, x, predictions, targets=None):
        """
        Arguments:
