...
class DenseCapRoIHeads(nn.Module):
    ...
    def select_training_samples(self, proposals, targets):
        ...
        gt_labels = [t['class'] for t in targets]

        # append ground-truth bboxes to propos
        # List[2*N,4],一个list是一张图片
        proposals_dense = [
            torch.cat((proposal.bbox, gt_box))
            for proposal, gt_box in zip(proposals, gt_boxes)
        ]
        for proposal, p, gt_label in zip(proposals, proposals_dense, gt_labels):
            proposal.bbox = p
            proposal.extra_fields['objectness'] = torch.cat(
                (proposal.extra_fields['objectness'], torch.ones_like(gt_label)), -1)

        # get matching gt indices for each proposal
        matched_idxs, labels = self.assign_targets_to_proposals(proposals_dense, gt_boxes, gt_labels)
        ...
        for img_id in range(num_images):
            ...
            proposals_dense[img_id] = proposals_dense[img_id][img_sampled_inds]

            proposals[img_id].bbox = proposals[img_id].bbox[img_sampled_inds]
            proposals[img_id].extra_fields['objectness'] = proposals[img_id].extra_fields['objectness'][
                img_sampled_inds]
        ...
        regression_targets = self.box_coder.encode(matched_gt_boxes, proposals_dense)
    ...
    def postprocess_detections(self, logits, box_regression, caption_predicts, proposals, image_shapes,
        ...
        for boxes, scores, captions, image_shape in zip(pred_boxes_list, pred_scores_list, pred_caption_list,
            ...
            captions = captions.unsqueeze(1).repeat((1, 1595, 1)).reshape(-1, 17)
        ...
        return all_labels, all_boxes, all_scores, all_captions, all_box_features
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        # proposals_dense = [p.bbox for p in proposals]
        # if self.training:
        #     proposals_dense, matched_idxs, caption_gt, caption_length, labels, regression_targets = \
        #         self.select_training_samples(proposals, targets)  # [512 512]
        # else:
        #     labels = None
        #     matched_idxs = None
        #     caption_gt = None
        ...
        if self.training:
            for i, p in enumerate(proposals):
                ...
                # anchors_in_image = []
                boxlist = BoxList(
                    targets[i]['boxes'], (image_width, image_height), mode="xyxy"
                )
                # anchors_in_image.append(boxlist)
                boxlist.add_field('labels', targets[i]['class'])
                boxlist.add_field('caps', targets[i]['caps'])
                boxlist.add_field('caps_len', targets[i]['caps_len'])
                # boxlist.add_field('labels', labels[i])
                bbox.append(boxlist)

        box_features, predictions, detector_losses = self.roi_heads(features,
                                                                    proposals, bbox)
        box_features = self.avgpool(box_features).view(box_features.size(0), -1)
        # features = {str(i):p for i,p in enumerate(features)}
        # box_features = self.box_roi_pool(features, proposals_dense,image_shapes)
        # box_features = self.box_roi_pool(features[0], proposals_dense)
        # box_features = self.box_head(box_features)  # [1024 2048]
        # logits, box_regression = self.box_predictor(box_features)
        logits, box_regression = detector_losses['loss_classifier'], detector_losses['loss_box_reg']
        if self.training:
            ...
            regression_targets = [box.extra_fields["regression_targets"] for box in proposals]
        ...
        if self.training:
            ...
            proposals_dense = [p.bbox for p in proposals]
            labels, boxes, scores, caption_predicts, feats = self.postprocess_detections(logits, box_regression,
                                                                                         caption_predicts,
                                                                                         proposals_dense,
