...
class BoxDescriber(nn.Module):
    ...
    def __init__(self, feat_size, hidden_size, max_len, emb_size, rnn_num_layers, vocab_size,
        ...
        # self.labels_embedding = nn.Embedding(1595, emb_size)
        # self.attrs_embedding = nn.Embedding(525, emb_size)
        # self.labels_embedding = nn.Embedding(vocab_size, emb_size)
        self.attrs_embedding = nn.Embedding(vocab_size, emb_size)
    ...
    def forward_train(self, labels, attrs, feat, cap_gt, cap_lens):
        """
        :param labels:
        :param attrs:
        ...
        # labels_emb=self.labels_embedding(torch.cat(labels,0).view(batch_size,-1))
        attr_emb=self.attrs_embedding(torch.cat(attrs,0).view(batch_size,-1))
        ...
        if self.fusion_type == 'init_inject':
            ...
            # _, (h0, c0) = self.rnn(labels_emb, (h0, c0))
            _, (h0, c0) = self.rnn(attr_emb, (h0, c0))
    ...
    def forward_test(self, labels, attrs, feat):
        ...
        if self.fusion_type == 'init_inject':
            ...
            # _, (h, c) = self.rnn(labels_emb, (h, c))
            _, (h, c) = self.rnn(attr_emb, (h, c))
    ...
    def forward(self, labels, attrs,feat, cap_gt=None, cap_lens=None):
        ...
        if self.training:
            ...
            return self.forward_train(labels, attrs,feat, cap_gt, cap_lens)
        else:
            return self.forward_test(labels, attrs,feat)
